...
class TestFimEnv(unittest.TestCase):
  ...
  def test_custom_uid_gid(self):
    ...
    out,err,code = self.run_cmd("fim-env", "add", "UID=5000", "GID=5000")
    ...
    out,err,code = self.run_cmd("fim-exec", "id", "-g")
    self.assertEqual(out, "5000")
    self.assertEqual(err, "")
    self.assertEqual(code, 0)
    # Verify full id output
    out,err,code = self.run_cmd("fim-exec", "id")
  ...
  def test_custom_home(self):
    ...
    out,err,code = self.run_cmd("fim-env", "add", "HOME=/opt/app/home")
    self.assertIn("Included variable 'HOME' with value '/opt/app/home'", out)
    self.assertEqual(err, "")
    self.assertEqual(code, 0)
    # Verify HOME environment variable
    out,err,code = self.run_cmd("fim-exec", "sh", "-c", "echo $HOME")
  ...
  def test_custom_shell(self):
    ...
    # Set custom shell
    out,err,code = self.run_cmd("fim-env", "add", "SHELL=/bin/sh")
  ...
  def test_custom_ps1(self):
    ...
    out,err,code = self.run_cmd("fim-env", "add", f"PS1={custom_prompt}")
    self.assertIn(f"Included variable 'PS1' with value '{custom_prompt}'", out)
    self.assertEqual(err, "")
    self.assertEqual(code, 0)
    # Verify PS1 in bashrc
    out,_,code = self.run_cmd("fim-exec", "sh", "-c", "cat $BASHRC_FILE 2>/dev/null")
  ...
  def test_root_identity_with_uid_zero(self):
    ...
    out,err,code = self.run_cmd("fim-env", "add", "UID=0", "GID=0")
  ...
  def test_root_mode_override(self):
    ...
    out,err,code = self.run_cmd("fim-root", "whoami")
    self.assertEqual(out, "root")
    self.assertEqual(code, 0)
    # Verify passwd entry for root
    out,err,code = self.run_cmd("fim-root", "cat", "/etc/passwd")
    self.assertIn("root:x:0:0:root:/root:", out)
    self.assertEqual(code, 0)
  ...
  def test_invalid_uid_fallback(self):
    ...
    out,err,code = self.run_cmd("fim-env", "add", "UID=notanumber")
  ...
  def test_invalid_gid_fallback(self):
    ...
    out,err,code = self.run_cmd("fim-env", "add", "GID=notanumber")
  ...
  def test_user_identity_reset(self):
    ...
    out,err,code = self.run_cmd("fim-env", "del", "UID", "GID", "USER", "HOME")
  ...
  def test_default_behavior_no_custom_values(self):
    ...
    out,_,code = self.run_cmd("fim-exec", "id", "-u")
    self.assertEqual(int(out), os.getuid())
    self.assertEqual(code, 0)
    # Should have some GID (host GID)
    out,_,code = self.run_cmd("fim-exec", "id", "-g")
    self.assertEqual(int(out), os.getgid())
    self.assertEqual(code, 0)
    # Should have a username (host username)
    out,_,code = self.run_cmd("fim-exec", "whoami")
    self.assertNotEqual(out, "")
    self.assertEqual(code, 0)
    # Should have HOME set
    out,_,code = self.run_cmd("fim-exec", "sh", "-c", "echo $HOME")
  ...
  def test_passwd_file_generation_custom(self):
    ...
    out,_,code = self.run_cmd("fim-exec", "test", "-r", "/etc/passwd")
    self.assertEqual(code, 0)
    # Verify passwd file contains our entry
    out,_,code = self.run_cmd("fim-exec", "cat", "/etc/passwd")
    self.assertEqual(code, 0)
  ...
  def test_combined_user_and_ps1(self):
    ...
    out,err,code = self.run_cmd("fim-exec", "sh", "-c", "cat $BASHRC_FILE")
